...
public class SparkBucketizeExecutor {
    ...
    public void execute() {
        ...
        Bucketizer bucketizer = new Bucketizer()
                ...
                .setOutputCol("bucket_col")
                .setSplits(splits);

        // Transform original data into its bucket index.
        Dataset<Row> bucketedData = bucketizer.transform(dataFrame);

        bucketedData = bucketedData.withColumn("bucket_col",
                bucketedData.col("bucket_col").cast(DataTypes.IntegerType));
    ...
    // Approach 1
    private Dataset<Row> getLabelledBucketedDfUsingDynamicExpression(final Dataset<Row> bucketedData) {
        ...
        for (int i = 0; i < replacementList.size(); i++) {
            resultColumn = functions.when(bucketedData.col("bucket_col").equalTo(i),
                    replacementList.get(i)).otherwise(resultColumn);
        }
    ...
    private Dataset<Row> getLabelledBucketedDfUsingJoin(final Dataset<Row> bucketedData) {
        ...
        Dataset<Row> bucketLabelsDf = this.sparkSession.createDataFrame(Arrays.asList(
        ...
        Dataset<Row> labelledBucketedData = bucketedData.join(bucketLabelsDf, "bucket_col")
                .select("features", "bucket_label");
